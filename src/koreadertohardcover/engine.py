...
class SyncEngine:
    ...
    def backfill_slugs(self) -> int:
        """
        Backfills missing hardcover_slugs for existing book mappings.
        Returns the number of updated records.
        """
        if not self.config.HARDCOVER_BEARER_TOKEN:
            logger.warning("HARDCOVER_BEARER_TOKEN not set. Cannot backfill slugs.")
            return 0
        ...
        with self.db.get_connection() as conn:
            # Find mappings without slugs
            mappings = conn.execute(
                "SELECT local_book_id, hardcover_id FROM book_mappings WHERE hardcover_slug IS NULL OR hardcover_slug = ''"
            ).fetchall()

            logger.info(f"Found {len(mappings)} mappings needing slug backfill.")

            for local_id, hc_id in mappings:
                try:
                    book = hc.get_book(int(hc_id))
                    if book and book.get("slug"):
                        conn.execute(
                            "UPDATE book_mappings SET hardcover_slug = ?, updated_at = now() WHERE local_book_id = ?",
                            [book["slug"], local_id],
                        )
                        updated_count += 1
                        logger.info(f"Updated slug for ID {hc_id}: {book['slug']}")
                    else:
                        logger.warning(f"Could not find slug for Hardcover ID {hc_id}")
                except Exception as e:
                    logger.error(f"Failed to backfill slug for {hc_id}: {e}")
