...
class InteractiveMapper:
    ...
    def map_book(self, local_id: str, title: str, author: str) -> Optional[str]:
        ...
        if len(shelf_results) == 1:
            ...
            click.echo(click.style(f"  Found exact match on your shelf: {selected['title']} ({selected['author_name']})", fg='green'))
            self.db.save_book_mapping(local_id, str(selected['id']), selected['title'], selected['author_name'])
        ...
        if len(shelf_results) > 1:
            ...
            click.echo(f"  Not found on shelf. Searching global Hardcover library for \"{title}\"...")
        ...
        for res in results:
            if res.get('canonical_id'):
                ...
                if self.client.is_book_on_shelf(canon_id):
                    click.echo(click.style(f"  Found canonical match on shelf: ID {canon_id} (linked to {res['title']})", fg='green'))
                    # Verify we want to map to canonical
                    if click.confirm(f"  Map to canonical book ID {canon_id}?", default=True):
                        self.db.save_book_mapping(local_id, str(canon_id), res['title'], res['author_name'])
                        return str(canon_id)
